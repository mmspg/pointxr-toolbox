    ...
    class PlyImporter : ScriptedImporter
    {
        ...
        public override void OnImportAsset(AssetImportContext context)
        {
            if (_containerType == ContainerType.Mesh)
            {
                ...
                //add new vector3[] to keep adaptive points
                Vector3[] fittedVertArr = DataBody.vertArr;
                modelName = Path.GetFileNameWithoutExtension(context.assetPath);

                var meshRenderer = gameObject.AddComponent<MeshRenderer>();
                meshRenderer.sharedMaterial = GetDefaultMaterial(modelName);

                //to fit the tranform of model to the vrScene
        ...
        //copy the defualtMaterial and rename it according to the model
        static Material GetDefaultMaterial(string name)
        {
            // Via package manager
            ...
            string path = "Assets/Resources/Materials/" + name + ".mat";  // copy the same rendering point material and rename it with the point cloud name
            AssetDatabase.CopyAsset(path_prj, path);
        ...
        class DataBody
        {
            ...
            public DataBody(int vertexCount)
            {
                ...
                //convert vertices into array for the use in FitTransform function
        ...
        void FitTransform(MeshRenderer mr, Transform transform, Vector3 desiredSize, ref Vector3[] fittedVertCal)//    desired size --> Vector3(1.414f, 1f, 1.414f)
        {
            Bounds b = mr.bounds;
            ...
            transform.localScale = Vector3.one * scale; // scale the surface 


            Bounds boundNew = mr.bounds;
            if (boundNew.extents.y * 2f > 1.75f)    // 1.75 is the maximum height ??
            {
                scale *= 1.75f / (boundNew.extents.y * 2f);
            }
            transform.localScale = Vector3.one * scale; //further tuning of scale based on vertical height of mesh
            Vector3 middlePos = b.center;
            Vector3 offset = (transform.position - middlePos) * scale; // scale the offset 
            ...
            {
                fittedVertCal[i] *= scale;// scale localscale
                fittedVertCal[i] += (offset + Vector3.up * mr.bounds.extents.y);// position
                if (bound.center.y < centerHeight - 0.1f)
                {
                    float off = centerHeight - transform.position.y;
                    fittedVertCal[i] += Vector3.up * off;  // lift the vertical position up 
                }
            }
        }
    }
